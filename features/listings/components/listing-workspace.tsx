...
export function ListingWorkspace({ context, filters, listings, errorMessage, consultationId }: ListingWorkspaceProps) {
  ...
  function applyFilters(formData: FormData) {
    ...
    ["q", "status", "transaction", "availability", "minDeposit", "maxDeposit", "holdingSource", "photo", "confirmedBefore"].forEach((key) => {
  ...
  function resetFilters() {
    const next = new URLSearchParams(searchParams.toString());
    ["q", "status", "transaction", "availability", "minDeposit", "maxDeposit", "holdingSource", "photo", "confirmedBefore"].forEach((key) => next.delete(key));
    router.replace(next.size ? `${pathname}?${next.toString()}` : pathname);
  }
    ...
    <section className="overflow-hidden rounded-xl border border-[#e5e1db] bg-white">
      <form className="border-b border-[#e5e1db] p-3" onSubmit={(event) => { event.preventDefault(); applyFilters(new FormData(event.currentTarget)); }}>
        <div className="flex flex-wrap gap-2">
          <input name="q" defaultValue={filters.query} className="h-9 min-w-56 flex-1 rounded-lg border border-[#e5e1db] px-3 text-xs outline-none focus:border-[#aa9b8d]" placeholder="건물명, 주소, 호실, 매물번호 검색" />
          <select name="status" defaultValue={filters.status} className="filter-field"><option value="active">관리 중 매물</option><option value="all">전체 상태</option>{Object.entries(statusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
          <select name="transaction" defaultValue={filters.transaction} className="filter-field"><option value="all">전체 거래</option>{Object.entries(transactionLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
          <select name="availability" defaultValue={filters.availability} className="filter-field"><option value="all">전체 입주 조건</option><option value="immediate">즉시 가능</option><option value="date_specified">날짜 지정</option><option value="needs_confirmation">확인 필요</option></select>
          <button type="submit" className="rounded-lg bg-[#3e3a37] px-3.5 text-xs font-bold text-white">검색</button>
          <button type="button" onClick={() => setShowMoreFilters((value) => !value)} className="rounded-lg border border-[#e5e1db] px-3 text-xs font-bold text-[#655f59]">상세 필터</button>
        </div>
        {showMoreFilters && <div className="mt-2 grid gap-2 border-t border-[#eeeae5] pt-2 sm:grid-cols-2 xl:grid-cols-5">
          <input name="minDeposit" defaultValue={filters.minDeposit} inputMode="numeric" className="filter-field" placeholder="최소 보증금 (만원)" />
          <input name="maxDeposit" defaultValue={filters.maxDeposit} inputMode="numeric" className="filter-field" placeholder="최대 보증금 (만원)" />
          <input name="holdingSource" defaultValue={filters.holdingSource} className="filter-field" placeholder="보유처" />
          <select name="photo" defaultValue={filters.photo} className="filter-field"><option value="all">전체 사진 상태</option>{Object.entries(photoLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
          <label className="filter-field flex items-center gap-2"><span className="shrink-0 text-[11px] text-[#7b7470]">재확인일 이전</span><input name="confirmedBefore" defaultValue={filters.confirmedBefore} className="min-w-0 bg-transparent text-xs outline-none" type="date" /></label>
        </div>}
...
function WorkspaceNotice({ title, description, error = false }: { title: string; description: string; error?: boolean }) {
  return <section className="rounded-xl border border-[#e8e1db] bg-white px-6 py-14 text-center"><div className={`mx-auto grid h-10 w-10 place-items-center rounded-lg font-mono text-sm font-bold ${error ? "bg-[#fff0ec] text-[#b94a42]" : "bg-[#f3e4dc] text-[#a85f43]"}`}>!</div><h2 className="mt-4 text-base font-extrabold">{title}</h2><p className="mx-auto mt-2 max-w-md text-sm text-[#7b7470]">{description}</p></section>;
}
