...
export async function getAdvertisingCopyTemplates() {
  ...
  const supabase = createSupabaseServerClient(); let query = supabase.from("advertising_copy_templates").select("id, property_group, template_name, title_template, body_template, is_active, updated_at").eq("organization_id", context.organizationId).order("property_group").order("template_name");
  if (context.role !== "admin") query = query.eq("is_active", true);
...
export async function saveAdvertisingCopyTemplate(values: AdvertisingCopyTemplateInput): Promise<Result> {
  ...
  const context = await getOrganizationContext(); if (context.kind !== "ready") return { ok: false, message: "활성 조직 멤버십을 확인할 수 없습니다." }; if (context.role !== "admin") return { ok: false, message: "문구 템플릿 관리는 관리자만 할 수 있습니다." };
