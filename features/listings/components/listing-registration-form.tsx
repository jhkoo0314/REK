  ...
  return <form className="mx-auto max-w-5xl space-y-5" onSubmit={form.handleSubmit(submit)}>
    <section className="rounded-xl border border-[#e5e1db] bg-white">
      ...
      <div className="p-5">
        ...
        {buildingMode === "existing" ? <div className="mt-4 space-y-4"><BuildingSearchPicker buildings={options.buildings} onSelect={(buildingId) => { form.setValue("buildingId", buildingId, { shouldDirty: true, shouldValidate: true }); form.setValue("unitId", ""); form.setValue("unitMode", "new"); }} selectedId={buildingId} /><FieldError message={form.formState.errors.buildingId?.message} />{options.sensitiveAccess.propertyContacts && <OwnerPhoneField form={form} />}</div> : <div className="mt-4 grid gap-4 md:grid-cols-2"><label><span className="label">건물명</span><input className="field" placeholder="예: 햇살하우스" {...form.register("buildingName")} /><FieldError message={form.formState.errors.buildingName?.message} /></label><label><span className="label">지번 주소</span><input className="field" placeholder="예: 배방읍 ○○리 123-4" {...form.register("lotAddress")} /></label>{options.sensitiveAccess.propertyContacts && <div className="md:col-span-2"><OwnerPhoneField form={form} /></div>}</div>}
      </div>
    </section>
    ...
    <section className="rounded-xl border border-[#e5e1db] bg-white">
      ...
      <div className="p-5">
        ...
        {unitMode === "existing" && buildingMode === "existing" ? <div className="mt-4 space-y-4"><label className="block"><span className="label">기존 호실</span><select className="field" {...form.register("unitId")}><option value="">호실 선택</option>{availableUnits.map((unit) => <option value={unit.id} key={unit.id}>{unit.unitNumber}</option>)}</select>{buildingId && availableUnits.length === 0 && <span className="mt-1 block text-xs text-[#a85f43]">선택한 건물에 등록된 호실이 없습니다. 새 호실을 입력해 주세요.</span>}<FieldError message={form.formState.errors.unitId?.message} /></label>{options.sensitiveAccess.unitAccess && <AccessPasswordField register={form.register("accessPassword")} />}{options.sensitiveAccess.propertyContacts && <TenantPhoneField form={form} />}</div> : <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3"><label><span className="label">호실</span><input className="field" placeholder="예: 302호" {...unitNumberField} onChange={(event) => { unitNumberField.onChange(event); const floor = inferFloorFromUnitNumber(event.target.value); if (floor) form.setValue("floor", floor, { shouldDirty: true }); }} /><FieldError message={form.formState.errors.unitNumber?.message} /></label><label><span className="label">층</span><input className="field" inputMode="numeric" placeholder="호실 입력 시 자동 입력" {...form.register("floor")} /><span className="mt-1 block text-[11px] text-[#7b7470]">예: 302호 → 3층. 필요하면 직접 바꿀 수 있습니다.</span></label><label><span className="label">방향</span><select className="field" {...form.register("direction")}><option value="">선택 안 함</option><option value="동향">동향</option><option value="서향">서향</option><option value="남향">남향</option><option value="북향">북향</option><option value="남동향">남동향</option><option value="남서향">남서향</option><option value="북동향">북동향</option><option value="북서향">북서향</option></select></label><label className="md:col-span-2 xl:col-span-1"><span className="label">고정 옵션</span><select className="field" {...form.register("elevatorOption")}><option value="">선택 안 함</option><option value="엘리베이터 있음">엘리베이터 있음</option><option value="엘리베이터 없음">엘리베이터 없음</option></select></label><div className="contents">{options.sensitiveAccess.unitAccess && <AccessPasswordField register={form.register("accessPassword")} />}{options.sensitiveAccess.propertyContacts && <TenantPhoneField form={form} />}</div></div>}
      </div>
    </section>
