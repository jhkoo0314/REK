...
function toPayload(values: ListingCreateInput) {
  return {
    ...
    normalizedAddress: normalizeForMatch(values.roadAddress || values.lotAddress),
    ...
    options: values.optionsText ? values.optionsText.split(",").map((item) => item.trim()).filter(Boolean) : [],
