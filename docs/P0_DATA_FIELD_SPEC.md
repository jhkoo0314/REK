...
- 연락처와 출입 비밀번호는 P0에서 별도 제한 테이블에 저장한다. 일반 목록·검색 결과·일반 화면 목업·광고 화면에는 넣지 않는다. Dev seed에는 저장·재조회 검증용으로 명백히 가공한 값 한 세트만 포함할 수 있다.
...
| `contact_note` | text | 아니오 | 연락 관련 짧은 업무 메모. 일반 목록·광고 화면 제외 |
...
| `access_note` | text | 아니오 | 비밀번호 외 출입 방법·주의 사항. 일반 목록·광고 화면 제외 |
| 감사 항목 | text/timestamptz | 예 | 공통 저장 규칙 적용 |

- 제한 정보는 매물 목록, 빠른 검색, 오늘 예정 업무, 광고 문구에서 절대 조회·표시하지 않는다. 단, Dev seed에는 상세 저장·재조회 검증을 위한 명백한 가공 값만 포함할 수 있다.
