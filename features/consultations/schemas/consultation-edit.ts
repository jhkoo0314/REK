...
export const consultationUpdateSchema = z.object({
  id: z.string().uuid(),
  category: z.enum(["general", "listing"]), initialListingId: z.string(), customerName: z.string().trim(), customerPhone: phone,
  consultationDate: z.string().min(1, "상담일을 입력해 주세요."), inflowSource: z.string().trim().min(1, "유입 경로를 선택해 주세요."), consultationMethod: z.enum(["phone", "message", "visit", "other"]), consultationNote: z.string().trim(),
  desiredAreas: z.array(z.string().trim()), desiredAreasOther: z.string().trim(), desiredRoomTypes: z.array(z.string().trim()), desiredRoomTypesOther: z.string().trim(), desiredDepositBudget: optionalAmount, desiredMonthlyRentBudget: optionalAmount, desiredMoveInDate: z.string(), requiredFeaturesNote: z.string().trim(),
  status: z.enum(["in_progress", "on_hold", "ended", "needs_confirmation"]), progressStage: z.enum(["new_inquiry", "condition_check", "visit_scheduled", "visit_completed", "reviewing", "closed"]), nextContactDate: z.string(), closedReason: z.string().trim(),
