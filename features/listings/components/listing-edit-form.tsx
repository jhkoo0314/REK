...
export function ListingEditForm({ editData }: { editData: ListingEditData }) {
  ...
  return <form className="mx-auto max-w-5xl space-y-5" onSubmit={form.handleSubmit(submit)}>
    <section className="rounded-xl border border-[#e5e1db] bg-white"><div className="border-b border-[#e5e1db] px-5 py-4"><p className="font-mono text-[11px] font-bold text-[#8b8279]">M-{String(listing.referenceNumber).padStart(6, "0")}</p><h2 className="mt-1 text-base font-extrabold">{listing.buildingName} {listing.unitNumber}</h2><p className="mt-1 text-xs text-[#7b7470]">건물·호실의 고정 정보는 건물·호실 관리에서 수정합니다.</p></div><div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3"><Field label="방 구조"><select className="field" {...form.register("propertyType")}><option value="one_room">원룸</option><option value="two_room">투룸</option><option value="two_bay">투베이</option><option value="three_room">쓰리룸</option><option value="owner_unit">주인세대</option></select></Field><Field label="매물 상태"><select className="field" {...form.register("listingStatus")}><option value="vacant">공실</option><option value="contract_in_progress">계약 진행</option><option value="contract_complete">계약 완료</option><option value="on_hold">보류</option></select></Field><Field label="거래 방식"><select className="field" {...form.register("transactionType")}><option value="monthly_rent">월세</option><option value="jeonse">전세</option><option value="to_be_confirmed">확인 필요</option></select></Field><Field label="보증금 (만원)" error={form.formState.errors.depositAmount?.message}><input className="field" inputMode="numeric" {...form.register("depositAmount")} /></Field><Field label="월세 (만원)" error={form.formState.errors.monthlyRentAmount?.message}><input className="field" disabled={transactionType === "jeonse"} inputMode="numeric" {...form.register("monthlyRentAmount")} /></Field><Field label="관리비 (만원)"><input className="field" inputMode="numeric" {...form.register("maintenanceFeeAmount")} /></Field><Field label="입주 가능 조건"><select className="field" {...form.register("availabilityType")}><option value="immediate">즉시 가능</option><option value="date_specified">날짜 지정</option><option value="needs_confirmation">확인 필요</option></select></Field>{availabilityType === "date_specified" && <Field error={form.formState.errors.availableDate?.message} label="입주 가능일"><input className="field" type="date" {...form.register("availableDate")} /></Field>}<Field label="퇴실 예정일"><input className="field" type="date" {...form.register("moveOutDate")} /></Field><Field label="사진 상태"><select className="field" {...form.register("photoStatus")}><option value="not_available">사진 없음</option><option value="available">사진 있음</option><option value="needs_confirmation">사진 확인 필요</option></select></Field><Field label="마지막 재확인일"><input className="field" type="date" {...form.register("lastConfirmedDate")} /></Field><label className="md:col-span-2 xl:col-span-3"><span className="label">보유처</span><input className="field" {...form.register("holdingSource")} /></label><Field label="세대 비밀번호 (제한 정보)"><input className="field" autoComplete="new-password" type="password" {...form.register("accessPassword")} /><span className="mt-1 block text-[11px] text-[#7b7470]">이 수정 화면에서만 확인·변경할 수 있습니다.</span></Field><Field label="임대인 연락처 (제한 정보)" error={form.formState.errors.ownerPhone?.message}><input className="field" inputMode="tel" type="tel" {...form.register("ownerPhone")} /></Field><Field label="세입자 연락처 (제한 정보)" error={form.formState.errors.tenantPhone?.message}><input className="field" inputMode="tel" type="tel" {...form.register("tenantPhone")} /></Field></div></section>
