...
P0 매물 관리와 P1 상담·계약·오늘 할 일의 실제 Dev DB 저장 흐름을 구현하고 확인했습니다. 계약 등록에서는 매물번호·건물명·주소·호수로 실제 계약 매물을 검색하고, 필요할 때 과거 매물 이력도 선택할 수 있습니다. 과거 이력 선택을 허용하는 Dev DB 변경 파일은 적용 대기 상태입니다. Excel·CSV 내보내기는 MVP에서 제공하지 않으며, Production 연결과 실제 업무 데이터 입력은 아직 하지 않습니다. 실제 구현 상태는 `docs/CURRENT_IMPLEMENTATION_STATUS.md`에서 확인합니다.
