...
export function ListingDetailView({ listing, readOnly = false }: { listing: ListingDetail; readOnly?: boolean }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><Link className="text-xs font-bold text-[#655f59] underline underline-offset-4" href={readOnly ? "/buildings" : "/listings"}>← {readOnly ? "건물·호실 관리" : "매물 목록"}</Link>{readOnly ? <span className="rounded-lg bg-[#f0eeea] px-4 py-2.5 text-xs font-bold text-[#716d67]">과거 이력 · 수정 불가</span> : <div className="flex flex-wrap gap-2"><ListingRetireButton listingId={listing.id} /><Link className="rounded-lg bg-[#3e3a37] px-4 py-2.5 text-xs font-bold text-white" href={`/listings/${listing.id}/edit`}>현재 매물 조건 수정</Link></div>}</div>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><InfoCard title="가격 조건"><InfoRow label={listing.transactionType === "sale" ? "매매가" : "보증금"} value={money(listing.depositAmount)} /><InfoRow label={listing.transactionType === "sale" ? "거래 방식" : "월세"} value={listing.transactionType === "sale" ? "매매" : listing.transactionType === "jeonse" ? "전세" : money(listing.monthlyRentAmount)} /><InfoRow label="관리비" value={money(listing.maintenanceFeeAmount)} /></InfoCard><InfoCard title="확인 상태"><InfoRow label="퇴실 예정일" value={date(listing.moveOutDate)} /><InfoRow label="보유처" value={listing.holdingSource ?? "—"} /></InfoCard><InfoCard title="건물·호실 고정 정보"><InfoRow label="층" value={listing.floor === null ? "—" : `${listing.floor}층`} /><InfoRow label="방향" value={listing.direction || "—"} /><InfoRow label="고정 옵션" value={listing.options.length ? listing.options.join(", ") : "—"} /></InfoCard>{listing.endReason && <InfoCard title="관리 종료 정보"><InfoRow label="종료일" value={date(listing.endDate)} /><InfoRow label="종료 사유" value={endReasonLabels[listing.endReason]} /></InfoCard>}</div>
    <section className="rounded-xl border border-dashed border-[#ded8d0] bg-[#fcfbf9] px-5 py-4 text-xs text-[#77736e]"><b className="text-[#514b45]">{readOnly ? "과거 이력 안내" : "연결 업무"}</b><p className="mt-1">{readOnly ? "이 화면은 당시 매물 조건을 확인하는 읽기 전용 기록입니다. 계약 결과와 연결 업무는 계약 관리 기능을 구현한 뒤 표시합니다." : "상담·계약 연결 요약은 해당 기능을 구현한 뒤 이곳에 표시합니다. 연락처·출입 정보는 이 상세 화면에 표시하지 않습니다."}</p></section>
  </div>;
}
