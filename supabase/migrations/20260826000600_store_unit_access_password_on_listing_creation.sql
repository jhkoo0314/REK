-- P0 follow-up: store an optional unit access password during new-listing registration.
-- Status: written only. Apply manually to Supabase Dev after 20260826000300.
-- Dev only: this stores fabricated passwords. Production needs RLS and its separate security review.
...
begin
  ...
  insert into public.listings (organization_id, unit_id, property_type, listing_status, is_current, transaction_type, deposit_amount, monthly_rent_amount, maintenance_fee_amount, available_date, move_out_date, exclusive_area_m2, photo_status, last_confirmed_date, field_status, holding_source, created_by_clerk_user_id, updated_by_clerk_user_id)
  values (p_organization_id, v_unit_id, (p_payload ->> 'propertyType')::public.property_type, (p_payload ->> 'listingStatus')::public.listing_status, true, (p_payload ->> 'transactionType')::public.transaction_type, nullif(p_payload ->> 'depositAmount', '')::integer, nullif(p_payload ->> 'monthlyRentAmount', '')::integer, nullif(p_payload ->> 'maintenanceFeeAmount', '')::integer, nullif(p_payload ->> 'availableDate', '')::date, nullif(p_payload ->> 'moveOutDate', '')::date, nullif(p_payload ->> 'exclusiveAreaM2', '')::numeric, (p_payload ->> 'photoStatus')::public.photo_status, nullif(p_payload ->> 'lastConfirmedDate', '')::date, (p_payload ->> 'fieldStatus')::public.field_status, nullif(p_payload ->> 'holdingSource', ''), p_clerk_user_id, p_clerk_user_id)
...
comment on function public.create_p0_listing is 'Creates a new building/unit/current listing and optionally stores the unit access password in the restricted table.';
