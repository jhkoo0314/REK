...
import { getSensitiveAccess, type SensitiveAccess } from "@/lib/auth/sensitive-access";
...
export async function getBuildingManagementData(): Promise<BuildingManagementData> {
  ...
  if (context.kind !== "ready") return { context, buildings: [], restrictedContactsAvailable: false, sensitiveAccess: { propertyContacts: false, unitAccess: false, consultationContacts: false } };
  ...
  const ownerByBuilding = new Map((ownerResult.data ?? []).map((item) => [item.building_id, sensitiveAccess.propertyContacts ? item.phone_number : ""]));
  const tenantByUnit = new Map((tenantResult.data ?? []).map((item) => [item.unit_id, sensitiveAccess.propertyContacts ? item.phone_number : ""]));
  const accessByUnit = new Map((accessResult.data ?? []).map((item) => [item.unit_id, sensitiveAccess.unitAccess ? item.access_password ?? "" : ""]));
  ...
  return { context, restrictedContactsAvailable: !tenantResult.error, sensitiveAccess, buildings: (buildingResult.data ?? []).map((item) => ({ id: item.id, name: item.name, roadAddress: item.road_address, lotAddress: item.lot_address, addressDetail: item.address_detail, postalCode: item.postal_code, ownerPhone: ownerByBuilding.get(item.id) ?? "", units: unitsByBuilding.get(item.id) ?? [] })) };
}
...
export async function updateManagedBuilding(values: BuildingUpdateInput): Promise<SaveResult> {
  ...
  if (sensitiveAccess.propertyContacts && !await saveOwnerPhone(supabase, context.organizationId, context.clerkUserId, value.id, value.ownerPhone)) return { ok: false, message: "건물 정보는 저장됐지만 임대인 연락처를 저장하지 못했습니다." };
...
export async function updateManagedUnit(values: UnitUpdateInput): Promise<SaveResult> {
  ...
  const { data: existingAccess, error: accessReadError } = sensitiveAccess.unitAccess ? await supabase.from("unit_access_details").select("id").eq("organization_id", context.organizationId).eq("unit_id", value.id).maybeSingle() : { data: null, error: null };
  ...
  if (!sensitiveAccess.propertyContacts) { revalidatePath("/buildings"); revalidatePath("/listings"); return { ok: true }; }
